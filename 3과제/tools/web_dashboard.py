...
CFG = {"namespace": "app", "waf_log_group": "aws-waf-logs-wsi2026", "waf_region": "ap-northeast-2"}
...
function diagnose(d){var t=[];
 d.apps.forEach(function(a){
  ...
  if(a.slo_rate<90)t.push(['bad',a.app+' 응답 지연 높음 — SLO '+a.slo_rate+'% (p99 '+a.p99+'ms, 목표 ≤'+a.slo_ms+'ms)',
   'replica/리소스로 분산하세요:\nkubectl scale deploy/'+a.app+' -n app --replicas=6\nkubectl patch hpa '+a.app+' -n app --type=merge -p \'{"spec":{"metrics":[{"type":"Resource","resource":{"name":"cpu","target":{"type":"Utilization","averageUtilization":35}}}]}}\'\nkubectl get pods -n app | grep Pending']);
  else if(a.slo_rate<99)t.push(['warn',a.app+' SLO 약간 미달 ('+a.slo_rate+'%)','replica 1~2개 증설 또는 HPA 임계값 하향 권장.']);
  if(a.e5>0)t.push(['bad',a.app+' 5xx '+a.e5+'건','DB/서버 오류. RDS CPU·커넥션 확인:\nkubectl logs -n app -l app='+a.app+' --tail=100 | grep 500']);
